...
bool BoostStore::Initialise(std::string filename, int type){
  ...
  if(type==0){
    ...
    if(file->is_open()){     
      ...
      if(!m_format){
        std::cout << "In BoostStore::Initialise --> type="<<type<<", m_format="<<m_format<<std::endl;
	...
	if(m_typechecking)ia & m_type_info;
        ...
        std::cout << "Into BoostStore::Initialise --> type="<<type<<", m_format="<<m_format<<std::endl;
	entryfile=filename;
...
void BoostStore::Save(std::string filename){
  ...
  if(m_format!=2){
    std::cout << "From BoostStore::Save ---> m_format "<< m_format << std::endl;
    ...
    if(!m_format){
      std::cout << "Using boost::archive::binary_oarchive " << std::endl;
      boost::archive::binary_oarchive oa(filter);
      oa & m_variables;
      if(m_typechecking)  oa & m_type_info;
    }
    else{
      std::cout << "Using boost::archive::text_oarchive " << std::endl;
